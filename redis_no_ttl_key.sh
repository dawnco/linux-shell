...
redis-cli -c -h $db_ip -p $db_port -a $password -n $dbname --no-auth-warning scan $cursor count $cnt > scan_tmp_result
...
do
    ttl_result=`redis-cli -c -h $db_ip -p $db_port -a $password -n $dbname --no-auth-warning ttl $line`  # 获取key过期时间
...
do
    ...
    echo "new_cursor $new_cursor"

    redis-cli -c -h $db_ip -p $db_port -a $password -n $dbname --no-auth-warning scan $new_cursor count $cnt > scan_tmp_result
    new_cursor=`sed -n '1p' scan_tmp_result`
    sed -n '2,$p' scan_tmp_result > scan_result
    cat scan_result |while read line
    do
        ttl_result=`redis-cli -c -h $db_ip -p $db_port -a $password -n $dbname --no-auth-warning ttl $line`
        if [[ $ttl_result == -1 ]];then
